...
修改：模型保存为 JSON + bin 格式（兼容 HuggingFace）+ 修复TensorBoard实时更新问题
"""
...
class GPTConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.n_head = n_head
        self.n_kv_head = n_kv_head or n_head // 3
        self.n_embd = n_embd
...
# ========== 新增：保存模型为 HF 格式（JSON + bin） ==========
def save_hf_format(model, config, save_dir, tokenizer=None, is_best=False):
    """
    保存模型为 HuggingFace 标准格式：
    - config.json: 模型配置
    - pytorch_model.bin: 模型权重
    - special_tokens_map.json/tokenizer_config.json: 分词器配置（可选）
    """
    # 创建保存目录
    ...
    logging.info(f"📝 保存配置文件: {os.path.join(save_dir, 'config.json')}")
    
    # 2. 保存模型权重（pytorch_model.bin）
    if hasattr(model, 'module'):
        model_state_dict = model.module.state_dict()
    else:
        model_state_dict = model.state_dict()
    
    # 过滤掉value_head（可选，推理时可移除）
    model_state_dict = {k: v for k, v in model_state_dict.items() if not k.startswith("value_head.")}
    
    torch.save(model_state_dict, os.path.join(save_dir, "pytorch_model.bin"))
    logging.info(f"💾 保存模型权重: {os.path.join(save_dir, 'pytorch_model.bin')}")
    
    # 3. 保存分词器（可选，方便后续推理）
    if tokenizer is not None:
        tokenizer.save_pretrained(save_dir)
        logging.info(f"🔤 保存分词器配置到: {save_dir}")
    
    # 4. 最佳模型添加软链接
    if is_best and os.path.exists(save_dir):
        best_link = os.path.join(os.path.dirname(save_dir), "best_model")
        ...
        os.symlink(save_dir, best_link)
        logging.info(f"🏆 创建最佳模型软链接: {best_link} -> {save_dir}")
...
def get_args():
    ...
    p.add_argument("--log_interval", type=int, default=1,  # 修改1：默认每1步记录（实时性）
                   help="TensorBoard 日志记录间隔（默认每1步记录一次）")
    p.add_argument("--flush_interval", type=int, default=1,  # 新增：强制刷盘间隔
                   help="TensorBoard 日志刷盘间隔（默认每1步刷盘）")
...
def main():
    ...
    if local_rank == 0:
        ...
        # 修改2：设置flush_secs=1（每秒自动刷盘），确保实时性
        writer = SummaryWriter(
            log_dir=args.log_dir,
            flush_secs=1  # 关键：每秒自动将缓冲区数据写入磁盘
        )
        logging.info(f"✅ TensorBoard 日志保存到: {args.log_dir} (自动刷盘间隔: 1秒)")
        # 记录超参数（新增）
        writer.add_hparams({
    ...
    # ========== 初始化模型配置（用于保存 config.json） ==========
    model_config = GPTConfig(
        vocab_size=args.vocab_size,
        n_layer=args.n_layer,
        n_head=args.n_head,
        n_kv_head=args.n_kv_head or args.n_head // 3,
        n_embd=args.n_embd,
    ...
    try:  # 修改3：添加异常捕获，确保writer正常关闭
        for epoch in range(args.epochs):
            ...
            for batch_idx, (prompt_ids, attention_mask) in enumerate(prompt_loader):
                ...
                # 每log_interval步记录日志和TensorBoard
                if iter_count % args.log_interval == 0 and local_rank == 0:
                    ...
                    if writer:
                        ...
                        # 修改4：强制刷盘，确保数据立即写入磁盘
                        if iter_count % args.flush_interval == 0:
                            writer.flush()
                            logging.debug(f"📝 TensorBoard 日志已刷盘 (step: {iter_count})")
                ...
                if iter_count % args.eval_steps == 0 and local_rank == 0:
                    ...
                    # ========== 修改：保存最佳模型为 JSON + bin 格式 ==========
                    if eval_avg_reward > best_reward:
                        best_reward = eval_avg_reward
                        best_save_dir = os.path.join(ckpt_dir, f"best_model_epoch{epoch}_step{iter_count}")
                        # 保存为 HF 格式
                        save_hf_format(
                            model=policy_engine.module,
                            config=model_config,
                            save_dir=best_save_dir,
                            tokenizer=tokenizer,
                            is_best=True  # 创建最佳模型软链接
                        )
                        logging.info(f"🏆 保存最佳模型到: {best_save_dir} (奖励: {best_reward:.4f})")
                
                # ========== 修改：保存检查点为 JSON + bin 格式 ==========
                if iter_count % 100 == 0 and local_rank == 0:
                    ckpt_save_dir = os.path.join(ckpt_dir, f"checkpoint_step{iter_count}")
                    # 保存检查点（HF格式）
                    save_hf_format(
                        model=policy_engine.module,
                        config=model_config,
                    ...
                    torch.save(train_state, os.path.join(ckpt_save_dir, "training_state.pt"))
                    logging.info(f"💾 保存检查点到: {ckpt_save_dir} (含训练状态)")
            ...
            if local_rank == 0:
                ...
                if writer:
                    ...
                    writer.flush()  # Epoch结束刷盘

    except Exception as e:  # 修改5：异常处理，确保writer正常关闭
    ...
    # ========== 修改：保存最终模型为 JSON + bin 格式 ==========
    if local_rank == 0:
        # 保存最终模型（HF格式）
        final_save_dir = os.path.join(ckpt_dir, "final_model")
        save_hf_format(
            model=policy_engine.module,
            config=model_config,
        ...
        logging.info(f"🎉 训练完成 | 最终模型保存到: {final_save_dir} | 最佳评估奖励: {best_reward:.4f}")
        ...
        if writer:
            ...
            writer.flush()  # 最终刷盘
            writer.close()
            logging.info(f"📝 TensorBoard 日志已保存至: {args.log_dir} (已关闭writer)")
